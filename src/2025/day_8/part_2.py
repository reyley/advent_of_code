...
def create_circuits(shortest_connections):
    for _connection in shortest_connections.ordered():
        ...
        if circuit1 and circuit2:
            ...
            Circuit({box1, box2})
        circuit1 = Circuit.node_to_circuit.get(box1)
        if len(circuit1.nodes) == len(Circuit.boxes):
...
def main(example=False):
    ...
    Circuit.boxes = []
    shortest_connections = MinHeap()
    for line in read_file(example):
        Circuit.boxes.append(tuple(int_line(line)))
    populate_shortest_connections(Circuit.boxes, shortest_connections)
    res = create_circuits(shortest_connections)
    print(res)
    return res


assert main(True) == 25272
main()